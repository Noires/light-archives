<template>
    <q-page class="page-violations">
        ...
        <q-table class="page-violations__table striped-list paged-link-table" :columns="columns" :rows="violations"
            row-key="name" v-model:pagination="pagination" wrap-cells @request="onPageRequest">
            <template v-slot:top>
                <q-form class="page-violations__filter">
                    <q-input class="page-violations__filter-search" v-model="searchQuery" label="Suche" debounce="200"
                        @update:model-value="refresh" />
                    <label class="page-violations__filter-open">
                        <span class="page-violations__open-label">Status:</span>
                        <q-select class="page-violations__open-select" v-model="open" emit-value map-options
                            :options="statusOptions" @update:model-value="refresh" />
                    </label>
                </q-form>
            </template>
...
export default class PageViolations extends Vue {
    ...
    async getLink(violation: ViolationSummaryDto) {
        ...
        if (violation.pageType === PageType.STORY) {
            ...
            await this.$router.push(`venue/${venue.server}/${venue.name}`);
        }
        else if (violation.pageType === PageType.WIKI_PAGE) {
            const wiki = await this.$api.wiki.getWikiPageById(violation.pageId);
            await this.$router.push(`wiki/${wiki.title}`);
        }
    }
